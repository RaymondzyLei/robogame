...
class PlaceModule:
    ...
    def __init__(self,
        ...
        self._placing_signal = signal(PlaceEvent.PLACING.value)
        self._task_complete_signal = signal(PlaceEvent.TASK_COMPLETE.value)
    ...
    def _handle_start_place(self, sender, request_id: str, data_key: str):
        """处理启动放置模块事件"""
        def handle_param(param):
            if param:
                print(f"[Place] Received place param: {param}")
    ...
    def _handle_place_check_done(self, sender, request_id: str, data_key: str):
        ...
        def handle_check_result(result):
            if result:
                print(f"[Place] Place check result: {result}")
    ...
    def _handle_module_retry(self, sender, request_id: str, data_key: str):
        ...
        def handle_retry_param(param):
            if param:
                ...
                if module == 'place':
                    print(f"[Place] Retrying, count: {param.get('retry_count', 0)}")
                    self._state = StatusCode.INIT
                    self._execute_init_state(param)
    ...
    def start(self):
        """启动放置模块"""
        self._running = True
        print("[Place] Place module started")
